...
def get_LVD_Summary_report(name, path):
    with open(name) as f:
        ...
        lvd_summ.columns = ['AZIMUTH', 'AVERAGE(U-VALUE/WINDOWS)', 'AVERAGE(U-VALUE/WALLS)', 'AVERAGE U-VALUE(WALLS+WINDOWS)', 
                          'WINDOW(AREA)', 'WALL(AREA)', 'WINDOW+WALL(AREA)']
        lvd_summ = lvd_summ[pd.to_numeric(lvd_summ['WINDOW+WALL(AREA)'], errors='coerce').notna()]
